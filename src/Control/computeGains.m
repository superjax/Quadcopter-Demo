...
% Roll loop
    phi_max = 45*pi/180; % Max error
    P.T_phi_max = .211;     % Max torque available (saturation limit)
...
% Pitch loop
    theta_max = 45*pi/180; % Max error
    P.T_theta_max = .211;  % Max torque available (saturation limit)
...
% Yaw rate loop
    psi_dot_max = 80*pi/180;
    P.T_psi_max = .105302; %N-m
...
% Altitude loop
    z_max = 1;     % Max error
    P.F_max = 1.688; % Max force available (saturation limit)
...
% x loop
    bsf_x = 8; % bandwidth seperation factor
    wn_x  = wn_pitch/bsf_x;
    z_x   = 0.9;
...
% y loop
    bsf_y = 8; % bandwidth seperation factor
    wn_y  = wn_roll/bsf_y;
    z_y   = z_x;
